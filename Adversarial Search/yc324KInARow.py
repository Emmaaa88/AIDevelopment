...
def makeMove(currentState, currentRemark, timeLimit=10000):
    ...
    newRemark = "Emmmmmmm..."
    move = minimax(currentState, 3, True, -float('inf'), float('inf'))[1][2]
    if move is None:
    ...
    if evaluation >= 10 ** (K + 1):
        ...
        newRemark = "This should be a good choice..."
    elif evaluation >= 10:
...
def checkCol(state, col):
    ...
    for row in range(M):
        ...
        if state[row][col] == I_PLAY:
            # check if they are neighbours with last same play, 
            # if so, the evaluation should be larger to pick this place first
